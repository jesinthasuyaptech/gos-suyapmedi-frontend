...
const taxmaster = () => {
  ...
  const fetchDatamedical = async () => {
    ...
    try {
      const response = await fetch(`${var_api}taxmaster/getby-hospital-medical/${hospital_id}`, {
        headers: {
  ...
  const handleSearch = (e) => {
    ...
    const filtered = data.filter((item) => {
        const taxType = taxTypeMapping[item.tax_type]?.toLowerCase() || ""; // Convert tax_type number to string value
        const taxName = item.tax_name?.toString().toLowerCase() || "";
        const itemValue = item.value?.toString().toLowerCase() || "";

        return taxType.includes(value) || taxName.includes(value) || itemValue.includes(value);
    });
  ...
  const handleFormSubmit = async (values) => {
    try {
      ...
      // Include the hospital_id dynamically in the form data
      let formData = { ...values, hospital_id,tax_type:1, value: parseInt(values.value, 0),remark: values.remark?.trim() || "-", };
  
      // For POST requests, add is_active: 1
      
        formData = { ...formData, is_active: editData?.is_active ||1 };
  ...
  const handleDeleteConfirm = (id,value) => {
    setDeleteId(id); 
    setDeletename(value)               // Set the ID of the rollmaster to delete
      ...
      render: (_, record) => (
        <div className="text-start">
          ...
          <button
            ...
            onClick={() => handleDeleteConfirm(record.id, record.value)}
          >
...
<Form.Item
  ...
  rules={[
    ...
    {
      validator: (_, value) =>
        value >= 0 ? Promise.resolve() : Promise.reject("Value must be positive!"),
    },
  ]}
>
  <input
...
/>
  ...
  >
    <Input />
     ...
     >
       <p>Are you sure you want to delete this Taxmaster value of  "<span style={{fontWeight:"bold"}}>{deletename}</span>"?</p>
       <Form.Item>
         <div className="d-flex justify-content-center">
           <button
             type="button"
