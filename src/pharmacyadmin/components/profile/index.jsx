...
import { Modal, Form, Input, notification, Radio, Select,Row,Col } from "antd";
import dayjs from "dayjs";
import { var_api, image_api } from "../../../constant";
import axios from "axios";
import hospitaldummy from "../../../admin/assets/img/hoslogo.png";
...
const Profile = () => {
  ...
  const user_id = localStorage.getItem("pres_user_id");
  ...
  const [showNewPassword, setShowNewPassword] = useState(false);
   const [showPassword, setShowPassword] = useState(false);
    ...
    const userRole = localStorage.getItem('pres_admin_role');
  ...
  const handleUpdate = async (values) => {
    ...
    try {
      const response = await fetch(`${var_api}nontechnicalstaff/update/${user_id}`, {
        method: 'PUT',
        headers: {
          'Authorization': ` ${localStorage.getItem('pres_token')}`, // Replace with actual token
        },
  ...
  const handlePasswordChange = async (e) => {
    ...
    try {
      const response = await fetch(`${var_api}nontechnicalstaff/admin/change-password/${user_id}`, {
        ...
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `${localStorage.getItem('pres_token')}`,
        },
  ...
  <a href="#" className="btn btn-primary" onClick={handleEditClickHospital}>
    ...
    <div className="col">
      ...
      <div className="row">
        ...
        <p className="col-sm-10">{adminjoinedDate}</p>
      </div>
      <div className="row">
