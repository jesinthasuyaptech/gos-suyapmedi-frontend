...
function DoctorNotification() {
  ...
  //patient notification data
    const fetchNotification = async () => {
      ...
      try {
        const response = await fetch(`${var_api}doctornotification/get-hospital/${hospital_id}/${doctor_id}`, {
          headers: {
 ...
 const markAsRead = async (notificationId) => {
  ...
  try {
    const response = await fetch(`${var_api}doctornotification/update/${notificationId.id}`, {
      method: 'PUT',
      headers: {
        "Content-Type": "application/json",
        Authorization: `${token}`,
  ...
  <>
      ...
      <Link to="#" className="nav-link p-0" onClick={handleShow}>
        {/* Notification Badge with Inline CSS */}
        <span 
          style={{
            position: "absolute", 
            top: "20px", 
        ...
        >
          {patientNotifications?.length > 99 ? "99+" : patientNotifications?.length}
        </span>
        <i className="fa-solid fa-bell" style={{ fontSize: "22px" }} />
      </Link>
    </li>

      {/* Modal for Notifications */}
      <Modal show={show} onHide={handleClose} centered size='xl' className='mt-4'>
