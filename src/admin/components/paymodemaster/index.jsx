...
const paymodemaster = () => {
              ...
              try {
                const response = await fetch(`${var_api}installation/update-hospital/${hospital_id}`, {
                  ...
                  body: JSON.stringify({
                    ...
                    servicetype: installDetails?.patient ?? false,
                ...
                if (response.ok) {
                  console.log("Installation techstaff updated successfully.");
                     if (installDetails?.paymodemaster !== 1) {
        setIsModalpop(true);
      }
                ...
                const fetchinstalldata = async () => {
                                ...
                                try {
                                  ...
                                  console.log("availabletime value:", firstItem?.paymodemaster);
  ...
  useEffect(() => {
    if (editData) {
      setEditIsActive(editData.is_active);
    }
  }, [editData]);
  ...
  const handleFormSubmit = async (values) => {
    try {
      ...
      if (!hospital_id) throw new Error("Hospital ID is not available.");
      setLoading(true);
  
      // Ensure that `editData.is_active` is passed correctly, even if it's 0
      const formData = {
        ...values,
        hospital_id,
        description: values.description || "-",
        is_active: editData?.hasOwnProperty('is_active') ? editData.is_active : 1, // Safe check for 0 and undefined
      };
  
      console.log("==== DEBUG SUBMIT START ====");
      console.log("editData:", editData);
      console.log("editData.is_active:", editData?.is_active);
      console.log("Form values (from Ant Form):", values);
      console.log("Final formData being sent:", formData);
      console.log("Final is_active being sent:", formData.is_active);
  ...
  <div><strong>Step 3:</strong> Click the <strong>Submit</strong> button to finalize the process.</div>
                            ...
                            <Modal
                             ...
                             footer={[
                               ...
                               onClick={() => {
                                 setIsModalpop(false);
                                 history.push('/admin/appointment-list'); // Call the switch case function here
                               }}
                             >
                               Continue
                             </Button>
