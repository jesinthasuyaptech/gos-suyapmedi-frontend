...
const Medicinesubcategory = () => {
  ...
  const [brands, setBrands] = useState([]); // State to store brands
    ...
    const uomOptions = [
    { value: 'tablet', uom_name: 'Tablet' },
    { value: 'capsule', uom_name: 'Capsule' },
    { value: 'ml', uom_name: 'Milliliter (ml)' },
    { value: 'mg', uom_name: 'Milligram (mg)' },
    { value: 'g', uom_name: 'Gram (g)' },
    { value: 'l', uom_name: 'Liter (l)' },
    { value: 'piece', uom_name: 'Piece' },
    { value: 'bottle', uom_name: 'Bottle' },
  ];
  const [uom, setUom] = useState(uomOptions); // State to store UOMs
 ...
 // Replace with your actual auth token
              ...
              console.error("Error updating techstaff:", err);
                ...
                const fetchinstalldata = async () => {
                    ...
                    try {
                      ...
                      console.log("availabletime value:", firstItem?.medicine);
  ...
  const handleFormSubmit = async (values) => {
    ...
    try {
      ...
      const formData = { ...values, hospital_id, remark: values.remark || "-", brand_id: 0, cat_id:0 };
...
>
  ...
  {uom.map((unit) => (
    <option key={unit.uom_name} value={unit.value}>
      {unit.uom_name}
    </option>
  ))}
</select>
  ...
  <div><strong>Step 3:</strong> Click the <strong>Submit</strong> button to finalize the process.</div>
                            ...
                            <Modal
                             ...
                             footer={[
                               ...
                               onClick={() => {
                                 setIsModalpop(false);
                                 history.push('/admin/paymodemaster'); // Call the switch case function here
                               }}
                             >
                               Continue
                             </Button>
